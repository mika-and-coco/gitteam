...
```bash
pip install -e ".[dev]"
pytest                    # 設定、能力マトリクス、規約、ペイロード、雛形、CLI、Web UI
```
...
CLI と同じ `commands/` 層をブラウザから操作できる画面です。すべての操作は「内容を確認する」→「この内容で実行する」の 2 段階で、確認だけなら GitHub にもローカルにも変更は加わりません。

```bash
start_ui.bat            # Windows（ダブルクリック可）。初回は仮想環境の作成と依存関係の導入も行います
./start_ui.sh           # macOS / Linux
streamlit run ui/streamlit_app.py   # 手動起動
```

| ページ | 内容 |
| --- | --- |
| はじめに | 4 ステップのセットアップ案内（ツールとログイン → 設定ファイル → メンバー登録 → 最初のリポジトリ） |
| 状態を見る | 設定・ログイン・ツールの状態、環境診断（doctor）、プランごとに使える機能 |
| チームの設定 | かんたん設定（フォーム）と詳細（YAML）の 2 タブで `gitteam.yaml` を編集 |
| PC のセットアップ | git 設定の適用、チームのリポジトリの一括クローン、現在の設定表示 |
| リポジトリを作る | `repo init` の実行（行うことを選択 → 確認 → 実行）、既存リポジトリへの適用 |
| チームとメンバー | 登録内容の一覧、GitHub への反映、一人だけの招待、GitHub 上の現状取得 |
| 日々の作業 | ブランチ名ビルダー、コミットメッセージ確認、PR 作成、リリース、自動チェック（hooks） |
| 用語集とよくある質問 | 画面に出る用語の説明と FAQ |

構成: `ui/streamlit_app.py`（エントリ、`st.navigation`）、`ui/app_pages/`（各ページ）、`ui/services.py`（コマンド呼び出し、出力キャプチャ、確認→実行フロー、日本語の操作要約）、`ui/glossary.py`（用語説明）。
ヘッドレステストは `tests/test_ui.py`（`st.testing.v1.AppTest`）にあります。

### 初心者向けの導線

* **はじめに**ページが最初に開き、「ツールとログイン → 設定ファイル → メンバー登録 → 最初のリポジトリ」の 4 ステップを進み具合付きで案内します。
* すべての操作は **「内容を確認する」→「この内容で実行する」** の 2 段階です。確認では「これから行う操作」を日本語の一覧で表示し、何も変更しません。
