...
_GIT_RULES: list[tuple[re.Pattern[str], Callable[[re.Match[str]], str]]] = [
    ...
    (re.compile(r"^git clone \S+ ('[^']*'|\S+)"), lambda m: f"{m.group(1).strip(chr(39))} にクローン"),
    (re.compile(r"^gh repo create (\S+) --(\w+)"), lambda m: f"リポジトリ {m.group(1)} を作成（{m.group(2)}）"),
    (re.compile(r"^gh repo clone (\S+)(?: ('[^']*'|\S+))?"), lambda m: f"{m.group(1)} を{(' ' + m.group(2).strip(chr(39)) + ' に') if m.group(2) else 'ローカルに'}クローン"),
